...
class TInvWL_Product {
	...
	function add_product( $data = array(), $meta = array() ) {
		$_data = filter_var_array( $data, array(
			...
			'quantity'     => FILTER_FORCE_ARRAY | FILTER_FLAG_ALLOW_FRACTION,
		) );

		if ( empty( $_data['quantity'] ) ) {
			$_data['quantity'] = 1;
		}
		if ( array_key_exists( 'quantity', $data ) && 0 === $data['quantity'] ) {
			$_data['quantity'] = 0;
		}
	...
	function add( $data = array(), $meta = array() ) {
		...
		$data    = apply_filters( 'tinvwl_wishlist_product_add_field', filter_var_array( $data, array(
			...
			'product_id'   => FILTER_VALIDATE_INT,
			'quantity'     => FILTER_SANITIZE_NUMBER_FLOAT | FILTER_FLAG_ALLOW_FRACTION,
			'variation_id' => FILTER_VALIDATE_INT,
			'wishlist_id'  => FILTER_VALIDATE_INT,
		) ) );
		$data    = array_filter( $data );

		if ( array_key_exists( 'quantity', $data_original ) && 0 === $data_original['quantity'] ) {
			$data['quantity'] = 0;
		}
	...
	function get( $data = array(), $count = false ) {
		...
		if ( ! empty( $data ) && is_array( $data ) ) {
			...
			foreach ( $data as $f => $v ) {
				$s = is_array( $v ) ? ' IN ' : '=';
				if ( is_array( $v ) ) {
					foreach ( $v as $_f => $_version ) {
						$v[ $_f ] = $wpdb->prepare( '%s', $_version );
					}
					$v = implode( ',', $v );
					$v = "($v)";
				} else {
