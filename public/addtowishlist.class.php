...
class TInvWL_Public_AddToWishlist {
		...
		if ( tinv_get_option( 'add_to_wishlist_catalog', 'show_in_loop' ) ) {
			switch ( tinv_get_option( 'add_to_wishlist_catalog', 'position' ) ) {
				case 'before':
					...
					add_action( 'ocean_before_archive_product_add_to_cart', 'tinvwl_view_addto_htmlloop' );
				...
				case 'above_thumb':
					...
					add_action( 'ocean_before_archive_product_image', 'tinvwl_view_addto_htmlloop' );
				...
				default: // Compatibility with previous versions.
					...
					add_action( 'ocean_after_archive_product_add_to_cart', 'tinvwl_view_addto_htmlloop' );
	...
	function add_to_wishlist() {
		...
		$wishlist_id   = absint( $post['tinv_wishlist_id'] );
		$wishlist      = null;
		$data          = array(
			'msg'        => array(),
			'status'     => false,
			...
			switch ( $post['product_type'] ) {
				...
				case 'grouped' :
					...
					if ( empty( $product ) ) {
						...
						$quantitys  = array_key_exists( 'quantity', $form ) ? filter_var( $form['quantity'], FILTER_VALIDATE_INT, FILTER_FORCE_ARRAY ) : array();
						foreach ( $variations as $variation_id ) {
							$quantity       = ( array_key_exists( $variation_id, $quantitys ) ) ? absint( $quantitys[ $variation_id ] ) : 1;
							...
							if ( $check_adding || ( tinv_get_option( 'general', 'simple_flow' ) && 'remove' === $post['product_action'] ) ) {
							...
							if ( tinv_get_option( 'general', 'simple_flow' ) && 'remove' === $post['product_action'] ) {
								if ( ! $allowed_adding && $wlp->remove_product_from_wl( 0, $post['product_id'], 0, apply_filters( 'tinvwl_addtowishlist_add_form', $form ) ) ) {
							...
							} elseif ( $check_adding && ! $allowed_adding ) {
				...
				case 'variable-subscription' :
					...
					$quantity       = floatval( array_key_exists( 'quantity', $form ) ? filter_var( $form['quantity'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ) : 1 );
					$quantity       = ( 0 < $quantity ) ? $quantity : 1;
					$check_adding   = false;
					$allowed_adding = false;
					if ( $single_product ) {
					...
					if ( tinv_get_option( 'general', 'simple_flow' ) && 'remove' === $post['product_action'] ) {
						...
						if ( ! $allowed_adding && $wlp->remove_product_from_wl( 0, $post['product_id'], $variation_id, apply_filters( 'tinvwl_addtowishlist_add_form', $form ) ) ) {
							$data['msg'][] = apply_filters( 'tinvwl_removed_from_wishlist_text', tinv_get_option( 'general', 'text_removed_from' ) );
	...
	function get_wishlists_data( $share_key ) {
		...
		if ( ( isset( $data['author'] ) && $data['author'] ) || $share_key ) {
			...
			$sql = "SELECT * FROM `{$table_lists}` WHERE ";

			if ( ( isset( $data['author'] ) && $data['author'] ) ) {
				$sql .= "`author` = '{$data['author']}'";
			} elseif ( $share_key ) {
				$sql .= "`share_key` = '{$share_key}'";
			}
			...
			if ( ! empty( $results ) ) {
				foreach ( $results as $wishlist ) {
					$user_wishlists[ $wishlist['ID'] ] = array(
						'ID'        => (int) $wishlist['ID'],
...
JOIN {$table_languages} l ON
	...
	function htmloutput_loop() {
		...
		if ( $product ) {
			if ( apply_filters( 'tinvwl_allow_addtowishlist_shop_loop_item', 'variable' !== $product->get_type() || tinv_get_option( 'add_to_wishlist_catalog', 'default_variation' ), $product ) ) { // @codingStandardsIgnoreLine WordPress.PHP.StrictInArray.MissingTrueStrict
				$this->is_loop = true;
				$this->htmloutput();
				$this->is_loop = false;
			}
	...
	function button( $echo = true ) {
		...
		if ( empty( $text ) ) {
			...
			if ( tinv_get_option( 'general', 'simple_flow' ) ) {
				$text = sprintf( '<span class="tinvwl_add_to_wishlist-text">%s</span><span class="tinvwl_remove_from_wishlist-text">%s</span>', $text, apply_filters( 'tinvwl_remove_from_wishlist_text_loop', tinv_get_option( 'add_to_wishlist' . ( $this->is_loop ? '_catalog' : '' ), 'text_remove' ) ) );
			} else {
		...
		if ( ! tinv_get_option( 'style', 'customstyle' ) ) {
			$icon .= ' tinvwl-skin-' . tinv_get_option( 'style', 'template' );
		}
