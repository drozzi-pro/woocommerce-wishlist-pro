...
class TInvWL_Public_Cart {
	...
	public static function add( $wishlist = null, $wl_product = 0, $wl_quantity = 1 ) {
		...
		// Skip processing and error in case of zero quantity.
		if ( 0 === $wl_quantity ) {
			return true;
		}
	...
	public function add_order_item_meta( $order ) {
		foreach ( $order->get_items() as $item ) {
			...
			$data = self::get_item_meta( $item->legacy_cart_item_key );
			if ( ! empty( $data ) ) {
				$item->update_meta_data( '_tinvwl_wishlist_meta', $data );
			}
		}
	}
	...
	/**
	 * Filter quantity in meta product
	 *
	 * @param array $data Meta data.
	 * @param string $cart_item_key Cart Item key.
	 * @param array $values Array with data of product.
	 *
	 * @return array
	 */
	function add_product_to_order( $data, $cart_item_key, $values ) {
	...
	/**
	 * Automatic removal products from wishlist
	 *
	...
	public function order_status_changed( $order_id, $old_status, $new_status ) {
		...
		if ( tinv_get_option( 'processing', 'autoremove' ) && tinv_get_option( 'processing', 'autoremove_status' ) == $new_status ) { // WPCS: loose comparison ok.
			$order = wc_get_order( $order_id );
			...
			foreach ( $items as $item ) {
				...
				$_product_meta  = $item->get_meta( '_tinvwl_wishlist_meta' );
				if ( ! empty( $_wishlist_cart ) && is_array( $_wishlist_cart ) ) {
					$_quantity = floatval( $item->get_quantity() );
					foreach ( $_wishlist_cart as $key => $quantity ) {
						if ( 0 >= $_quantity ) {
							break;
						}
						$wishlist = $this->get_order_wishlist( $key, $order->get_customer_id() );

						$_quantity = self::ar_f_wl( $wishlist, $item->get_product_id(), $_quantity, $item->get_variation_id(), $_product_meta );
					}
					$item->delete_meta_data( '_tinvwl_wishlist_cart' );
				}
			}
		} // End if().
	}
	...
	private static function ar_f_wl( $wishlist, $product_id, $quantity = 1, $variation_id = 0, $meta = array() ) {
		...
		if ( $quantity >= $product['quantity'] ) {
			if ( $wlp->remove_product_from_wl( 0, $product_id, $variation_id, $product['meta'] ) ) {
				$quantity -= $product['quantity'];
				do_action( 'tinvwl_changed_wishlist', 4, $wishlist, $product_id, $variation_id, $product['quantity'] );
			}
		} else {
			$product['quantity'] -= $quantity;
			if ( $wlp->update( $product ) ) {
				do_action( 'tinvwl_changed_wishlist', 4, $wishlist, $product_id, $variation_id, $quantity );
